...
| **4** | Unity UI 바인딩 (텍스트/도형만, 2~3화면) | **✅ 스코프 완료 (2026-09-24 사용자 선언 · `main` 병합)** — 브랜치 `ui/combat-view`. **✅ 첫 화면(무공 목록·상세) 완료 — Play 4차 사용자 확인.** 한글 · 런타임 조립(에디터 작업 **0**) · Core 바인딩 · 스크롤 · 필터 · 표가 전부 섰다. `dotnet test` **293/0/0** · 기준선 791건 동일. **✅ 전투 화면 — 장착 규정(§0-1) 적용 · Play 확인 통과**(2026-09-24). 슬롯은 **종류당 1개**(공격·내공·경공)다. ⚠ 슬롯의 `1/3` 표시가 종류별 점유를 안 보여 준다 — 미착수(HANDOFF §4-18-5). 경위 [`HANDOFF §4-16`](../../docs/HANDOFF.md) · 설계 [`ui-martial-list-plan.md`](../../docs/ui-martial-list-plan.md). **✅ 범위 대가 T = 1.30 확정 + 무공 2종 개명**(2026-09-24 · HANDOFF §4-19). ⏸ `만우쾌사`(전승 유일 범위)는 어떤 조합으로도 35 미만이라 별건으로 남는다 |

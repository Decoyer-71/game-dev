...
namespace Jianghu.Tests.Combat
{
    ...
    public class BasicStrikeRateTests
    {
        ...
        public void 형태소가_많은_무공일수록_더_자주_평타로_내려앉는다()
        {
            ...
            //   통과로 위장되면 다음 세션이 "기력 축은 검증됐다" 고 믿는다.
            //
            // ⚠⚠ **`Assert.Inconclusive` 가 아니라 `Assert.Ignore` 를 쓴다** (2026-08-01 실측 교정).
            //   Inconclusive 로 뒀더니 `dotnet test` 는 "건너뜀" 인데 **Unity Test Runner 는 빨강(실패)**
            //   으로 집계했다(실측 ✅161·❌2). 빨강이 "고장" 을 뜻하지 않게 되면 경보가 무의미해진다.
            //   NUnit3 결과 모델에서 `Ignored` 는 `Skipped` 카테고리이고 `Skipped` 는 `Failed` 와
            //   **별개 최상위 값**이라, 두 러너 모두에서 실패로 집계되지 않는다.
            // ⚠ **정적 `[Ignore]` 어트리뷰트를 쓰지 않는다.** 그러면 기력 축이 살아나도 영영 안 돈다.
            //   조건부 호출이라 축이 살아나는 순간 자동으로 진짜 Pass/Fail 로 승격된다.
            if (four <= 0.05)
            {
                Assert.Ignore(
        ...
        public void 권_숙달자는_같은_무공을_들어도_덜_마른다()
        {
            ...
            // ⚠ `Assert.Ignore` 를 쓰는 이유는 위 테스트의 주석 참조 (Unity 는 Inconclusive 를 빨강으로 센다).

...
namespace Jianghu.Core.Martial.Morphemes
{
    ...
    public static class MorphemeDictionary
    {
        ...
        /// ✅⚠⚠ **1.30 으로 확정했다** (2026-09-24 · 사용자 확정 · `verify` 2회 · HANDOFF §4-19).
        ///   ~~1.15~~ 는 파단점(1.22) **아래**라 범위가 구조적으로 지는 값이었다 — 동질 4인 팀이
        ///   §D8 이 못박은 *"범위 가치의 **상한**"* 인데, 그 상한에서조차 `범위이득` 이 **−19.1%p**(10성)였다.
        ///
        /// ⚠⚠ **단독으로 정한 값이 아니다.** 같은 커밋에서 무공 두 종의 형태소를 함께 바꿨다
        ///   (`정천창군`→`유천창군` · `환창혈만`→`정창혈만`). **T 만 올리면 이미 센 것이 같이 올라간다** —
        ///   종별 편차를 카탈로그로 먼저 좁힌 뒤에야 이 값이 성립한다. 둘은 한 몸이므로 **따로 되돌리지 말 것.**
        ///
        /// ⚠ **전수 사망 0 을 만족하는 T 는 못 찾았다.** 버킷 전수(25종×3경지)에서 개선 12 · 악화 3 이고
        ///   악화 셋은 전부 문턱 근소 초과다 — `궤야척혈` 3성 34.37(−0.63) · `유천창군` 6성 68.44(+3.44) ·
        ///   `환벌혈군` 3성 69.53(+4.53). ⚠ T=1.25 는 **더 나쁘다**(`후참혈군` 10성이 새로 죽는다).
        /// ⚠ 이 수치는 **4대4 동질 팀 상한 측정**이지 실전 밸런스 판정이 아니다(§D8).
        /// </summary>
        public const double ScopeDamageBudget = 1.30;

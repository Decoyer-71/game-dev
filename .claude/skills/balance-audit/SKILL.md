...
- **기준선에 이미 있는 문제는 diff 로 안 보인다.** 지금 기준선에도 대문파 격차 같은 미해결 항목이 들어 있다. 그건 별건으로 남는다
- ⚠⚠ **계층 판정 기준이 바뀌었다 (2026-08-05 · HANDOFF §4-9-7).** `계층 내 격차 ≤ 30%p` → **`평균 ± 2σ ⊂ [35, 65]`**. 격차는 **종수에 체계적으로 끌려가** 5종짜리 강호무학과 39종짜리 대문파를 같은 잣대로 잴 수 없었다.
  → ⛔ **문서에 남은 옛 `✅ 격차 … 목표 이내` 표시를 그대로 믿지 말 것.** 예: 전승무학이 격차 29.7 로 ✅ 였는데 그 계층에 `마한중참` 67.1(지배 의심)이 있었고, 새 기준 상단 2σ 는 **74.9 ⚠** 다
- ⚠ **강호무학(5종)·전승무학(7종)은 σ 추정이 불안정하다.** 도구가 `⚠ 표본 N종` 경고를 붙인다 — **그 두 계층의 ✅/⚠ 를 정책 결정의 유일한 근거로 쓰지 말 것**
- ⚠ **승률 분포가 정규인지 검정한 적이 없다.** `±2σ ≈ 95%` 는 가정이며, 그래서 이 값도 **판정이 아니라 신호**다

...
| **Python 3** | — | PATH 등록 | `.claude/hooks/` **네 개**(로그 가드 · 밸런스 가드 · **세션 배너** · **브랜치 가드**)를 `python` 명령으로 호출한다. ⚠ 세션 배너는 `docs/` 를 스캔해 문서 인덱스를 주입한다(`../CLAUDE.md` §10) — **이게 안 돌면 세션이 문서 목록을 못 본다** |
...
1. `.claude/settings.json` — `hooks[].command` 의 경로 **4개** + `permissions.allow` 의 dotnet 경로 **6개**(PowerShell 4 · Bash 2)
